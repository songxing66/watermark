...
export default class Watermark {
  constructor(setting) {
    ...
    this.watermarkDom = new MutationObserver(this.callback.bind(this));
  ...
  _loadMark(settings) {
    ...
    /*如果元素存在则移除*/
    var watermark_element = this.otdiv;
    if (watermark_element && watermark_element.parentNode) {
    ...
    /*创建水印外壳div*/
    var otdiv = this.otdiv;
    var shadowRoot = null;
    if (!otdiv) {
      otdiv = $creactWaterNode(defaultSettings.id || '');
    ...
    const minotor =
      settings.monitor === undefined
        ...
        : settings.monitor;
    if (minotor) {
      this.watermarkDom.observe(watermark_hook_element, this.option);
      this.watermarkDom.observe(otdiv.shadowRoot, this.option);
    }
  }
  ...
  init(settings) {
    ...
    this.bindsome = this._ListenerCallback.bind(this);
    window.addEventListener('onload', this.bindsome);
    window.addEventListener('resize', this.bindsome);
  }
  destroy() {
    this.watermarkDom.disconnect();
    window.removeEventListener('onload', this.bindsome);
    window.removeEventListener('resize', this.bindsome);
    this._removeMark();
  }
  _ListenerCallback() {
    const settings = this.globalSetting;
    this._loadMark(settings);
  }
  /*手动加载水印*/
  load(settings) {
